  ...
  return (
    <PageShell accent="violet">
      <AnimatedSection className="mb-6 grid gap-5 lg:grid-cols-[1.1fr_.9fr]">
        <div><Badge>Smart CRM</Badge><h1 className="mt-4 text-4xl font-semibold md:text-6xl">Центр управления выручкой для премиальных отделов продаж.</h1><p className="mt-4 max-w-2xl text-slate-400">Визуальная CRM с клиентами, сделками, воронкой, уведомлениями, сообщениями, календарем и статической аналитикой.</p></div>
      ...
      <div className="grid gap-5 xl:grid-cols-[340px_1fr]">
        ...
        <div className="space-y-5">
          ...
          <Card><h3 className="mb-4 font-semibold">Карточки клиентов</h3><div className="grid gap-3 md:grid-cols-4">{clients.map((client) => <button key={client.name} onClick={() => setSelected(client)} className="rounded-2xl bg-white/[.05] p-4 text-left hover:bg-white/[.09]"><UserRound className="mb-3 h-5 w-5 text-cyan-300" /><b>{client.name}</b><p className="text-sm text-slate-400">{client.owner}</p></button>)}</div></Card>
        </div>
      </div>
    </PageShell>
  );
