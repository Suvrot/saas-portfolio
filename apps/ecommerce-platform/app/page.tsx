  ...
  return (
    <PageShell accent="rose">
      <AnimatedSection className="mb-6 grid gap-5 lg:grid-cols-[1.15fr_.85fr]">
        <div><Badge>E-Commerce Платформа</Badge><h1 className="mt-4 text-4xl font-semibold md:text-6xl">Премиальный интернет-магазин с визуальной админ-аналитикой.</h1><p className="mt-4 max-w-2xl text-slate-400">Статический каталог товаров, поиск, категории, карточка товара, корзина и панели продаж в стиле админки.</p></div>
        <Card className="relative overflow-hidden"><div className="absolute inset-0 bg-gradient-to-br from-rose-400/20 to-cyan-400/10" /><div className="relative"><ShoppingBag className="mb-8 h-10 w-10 text-rose-300" /><p className="text-5xl font-semibold">$42.8K</p><p className="text-slate-400">Демо-продажи за месяц</p><Bars data={[20, 34, 25, 48, 52, 64, 58]} color="from-rose-400 to-orange-500" /></div></Card>
      </AnimatedSection>
      <div className="grid gap-5 xl:grid-cols-[1fr_360px]">
        <div className="space-y-5">
          ...
          <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">{filtered.map((product) => <Card key={product.id} className="group cursor-pointer" onClick={() => setSelected(product)}><div className={`mb-5 h-44 rounded-3xl bg-gradient-to-br ${product.color} opacity-90 shadow-2xl transition group-hover:scale-[1.02]`} /><div className="flex items-start justify-between gap-3"><div><h3 className="font-semibold">{product.name}</h3><p className="text-sm text-slate-400">{product.category}</p></div><b>${product.price}</b></div><Button className="mt-4 w-full" variant="secondary" onClick={(event) => { event.stopPropagation(); setCart([...cart, product.id]); }}><Plus className="h-4 w-4" /> В корзину</Button></Card>)}</div>
        </div>
        <div className="space-y-5">
