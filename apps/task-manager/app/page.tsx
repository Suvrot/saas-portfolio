...
export default function TasksPage() {
  ...
  return (
    <PageShell accent="violet">
      <AnimatedSection className="mb-6 flex flex-col justify-between gap-5 lg:flex-row lg:items-end"><div><Badge>Платформа управления задачами</Badge><h1 className="mt-4 text-4xl font-semibold md:text-6xl">Рабочая система в стиле Linear для сфокусированных команд.</h1><p className="mt-4 max-w-2xl text-slate-400">Перетаскивайте карточки, фильтруйте задачи, открывайте модальное окно, таблицу, календарь, проекты, команду, приоритеты, статусы и labels.</p></div><Button variant="premium"><Layers3 className="h-4 w-4" /> Создать задачу</Button></AnimatedSection>
      <div className="grid gap-5 lg:grid-cols-[260px_1fr]">
        <Card className="space-y-3"><b>Рабочее пространство</b>{navItems.map(({ Icon, label }) => <button key={label} className="flex w-full items-center gap-3 rounded-2xl px-3 py-3 text-sm text-slate-300 hover:bg-white/[.07]"><Icon className="h-4 w-4 text-violet-300" />{label}</button>)}</Card>
